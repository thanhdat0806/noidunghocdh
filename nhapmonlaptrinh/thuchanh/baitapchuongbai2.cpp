...
//Ham tra ve tong cac chu so NT cua n
...
int main()
{
	...
	printf("Tong cac chu so NT cua %d = %d .\n", n, TongCSNT(n)); 
	printf("Tong cac chu so CP cua %d = %d .\n", n, TongCSNT(n));               
}
...
unsigned int TongCSCP(unsigned int n){
	unsigned int sum = 0;
	unsigned char chusocuoi;
	
	while(n > 0){
		chusocuoi = n % 10;
		if(LaSoChinhPhuong(chusocuoi))
			sum += chusocuoi;
		n /= 10;
	}
	return sum;
